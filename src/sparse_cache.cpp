#include "sparse_cache.h"
#include "pin.H"

#include <cstdlib>
#include <time.h>

SparseCache::SparseCache(uint32_t _numTagLines, uint32_t _numDataLines, CC* _cc, SparseTagArray* _tagArray, SparseDataArray* _dataArray,
...
TimingCache(_numTagLines, _cc, NULL, tagRP, _accLat, _invLat, mshrs, 5, ways, cands, _domain, _name, _evStats, _tag_hits, _tag_misses, _tag_all), numTagLines(_numTagLines), numDataLines(_numDataLines),
tagArray(_tagArray), dataArray(_dataArray), tagRP(tagRP), dataRP(dataRP), crStats(_crStats), evStats(_evStats), tutStats(_tuStats), dutStats(_duStats) {
        srand (time(NULL));
}

void SparseCache::initStats(AggregateStat* parentStat) {
    AggregateStat* cacheStat = new AggregateStat();
    cacheStat->init(name.c_str(), "sparse cache stats");
...
void SparseCache::initCacheStats(AggregateStat* cacheStat) {
...
uint64_t SparseCache::access(MemReq& req) {
    ...
    if (likely(!skipAccess)) {
        ...
        if (tagId == -1) {
            ...
            if (approximate) {
                ...
                uint32_t map = dataArray->computeMap(data, type, min, max);
            ...
            if (approximate && req.type == PUTX) {
                ...
                if (map == previousMap) {
                    ...
                    if (mapId != -1) {
                        ...
                        HitWritebackEvent* hwe = new (evRec) HitWritebackEvent(this, he, 2*accLat, domain);
...
void SparseCache::simulateHitWriteback(HitWritebackEvent* ev, uint64_t cycle, HitEvent* he) {
    uint64_t lookupCycle = tryLowPrioAccess(cycle);
    if (lookupCycle) {
        if (!pendingQueue.empty()) {
            // assert(lookupCycle <= pendingQueue.top().cycle);
